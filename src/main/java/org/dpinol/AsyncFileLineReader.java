...
/**
 * Uses prefetching. It's ~20% slower than without it
 * Previous version with AsyncFileChannel was much slower. Discarded
...
public class AsyncFileLineReader implements AutoCloseable {
    ...
    public AsyncFileLineReader(Path input) throws IOException {
        fileChannel = FileChannel.open(input, Collections.singleton(StandardOpenOption.READ), new FileAttribute[0]);
        this.input = input;
        readBufferAsync();
    }
    ...
    public void readBufferAsync() {
        ...
        CompletableFuture.runAsync(() -> {
            try {
                int bytesRead = fileChannel.read(buffer, fileOffset);
                fileOffset += bytesRead;
                readBufferCf.complete(bytesRead);
            } catch (IOException e) {
                readBufferCf.completeExceptionally(e);
            }
        });
    }

    public CompletableFuture<FileLine> readAsync() {
        CompletableFuture<FileLine> lineCf = new CompletableFuture<>();
        CompletableFuture.runAsync(() -> {
            try {
                FileLine fileLine = readLine();
                if (fileLine != null) {
    ...
    public FileLine readLine() throws Exception {
        lineHead.reset();
        lineLength.set(0);
//        if (bufferSize < 0) {
...
//        }
        ...
        if (lineLength.get() <= Global.BUFFER_SIZE)
            ...
            return new LongLine(fileChannel, lineHead.toString(), curStartOffset, lineLength.get());
    }
